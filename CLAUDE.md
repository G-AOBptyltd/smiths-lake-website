...
- **Session 18a (Aug 20):** **Validation + enforcement deployed** — prevent empty story publication. Root cause of "Smiths Lake - Community Day shows Live but has no content" was **lack of validation**: the news article was created + published without a summary or page body. Fix: `news-save.js` now rejects `live=true` without description; `news-toggle.js` rejects enabling feed visibility without description or body blocks. Double enforcement (save + toggle) prevents silent failures. Merged `worktree-news-body-render` to main (advertising, services, events modules + validation). Created `/api/news-fetch` GET endpoint for runtime Notion page fetching (future use). Tested validation live on deployed site — error message displays correctly. **Standing rule:** editorial platforms with draft→live flows need validation at BOTH checkpoints (accept + enable), not display-time fallbacks.
- **Session 18 (Aug 21):** **News admin form bug fix + article recovery.** Root cause: `saveStory()` was calling `/api/news-body` with empty `blocks` array, which cleared the Notion page body when editors used the simple "Write a story" form. Problem: that form is metadata-only (headline/date/photo) and should never touch the page body. Fix: removed the news-body call from `saveStory()` in `public/admin/news/index.html`, leaving page-body management exclusively to the template composer. Restored the "Name the Bridge - Our new community asset" article by adding content + survey link back to its Notion page body via the Notion API. Changes deployed live (article mostly working; image rendering has a separate issue to debug later). **Architectural lesson:** forms touching multiple data layers need explicit fail-safe defaults — metadata-only forms must never write to page bodies.

- **Session 19 (Aug 24):** **Homepage ticker fixes + News Desk rich-body edit guard + homepage layout move** (3 merges to main, all verified live). (1) **Ticker supporter shout-outs were invisible** ("SUPPORTER · blank · SUPPORTER"): the JS-injected supporter cards inherited navy text on the navy ticker band because Astro's SCOPED `<style>` (`.ticker-hed{color:#fff}` etc.) never reaches `createElement()` nodes. Fix: inline the visible styles on the injected elements in `src/pages/index.astro`. (2) **Ticker category labels for Project Hub / Services were invisible** — their section colour is navy `#1B365D`, identical to the ticker band. Fix: a ticker-only `tickerCatColour()` helper that lifts any accent with luminance <90 by blending 55% toward white (bright colours like News teal pass through); global `SECTION_COLOURS` untouched. (3) **News Desk rich-body edit guard:** `/news/[slug]` renders the Notion PAGE BODY when present, falling back to Description only when the body is empty — so editing a rich-composer story's text via the simple metadata-only "Edit story" form silently doesn't change the article. Added a `GET` mode to `news-body.js` (returns `{ blockCount, hasRichBody }`, mirrors `NotionPageContent`'s empty-paragraph filter, fail-open) and an amber preflight notice in `/admin/news/` shown when the opened story has a page body (spells out that headline/date/photo DO apply but the "Your story" text won't). (4) **Layout:** moved the 📰 Village News banner + 🏆 "Who's supporting us this month" supporters card to sit directly below the news ticker and above the "Explore Our Community" grid. **Non-bug note:** a reported "news update didn't publish" was a false alarm — the edit was fully live; the confusion was the static-build publish lag (News Desk Save writes Notion instantly, but the site only changes on the separate "Publish to website" build-hook + ~2-3 min + CDN cache).

## Projects — system of record (/admin/projects/, added Session 21, Aug 2026)
The spine that ties **grants + co-contribution budget/schedule + volunteer groups (and their hours)** into one auditable project, with steering-committee reports. Built by ELEVATING the existing Co-Contribution project entity — **no new project tables**.
- **Data model:** a "Project" IS the cocon Projects row (`NOTION_COCON_PROJECTS_DB_ID`). `_projects.js` self-heals new fields onto it: `Volunteer Groups` (JSON [{path,title}]), `Volunteer Hour Rate`, `Exec Summary`, `Description`, `Lead`, `Hub Slug` (JSON array of Project-Hub slugs — `parseHubSlugs` reads a legacy single slug too), `Publish to Hub` (checkbox). Budget/Schedule stay in the cocon Schedule/Budget DBs keyed by project slug; grants link by the grant's `Project` = slug.
- **Functions:** `projects-list`, `projects-get` (drill-in aggregator: project + schedule + budget + rollups[computeRollups] + grants-by-slug + volunteer-hours + `exec` + `availableGroups`), `projects-lifecycle` (archive→Status=Archived / restore→Active [admin·pm] / delete→Notion trash [admin only]), `project-hub-publish` (creates/refreshes a DRAFT Project-Hub content card per linked slug, schema-introspecting), `project-link-group` (volunteer-side reconcile [admin·pm]), `project-link-grant` (set a grant's Project [admin·pm]). Project metadata writes reuse `cocon-save` (entity=project; grant-request amount stays admin/treasurer). Shared `_projects.js` `listGroups()` = **all published volunteer-section cards** (Environment/Groups/Emergency) ∪ cards seen in VF Volunteers/Activities — PII-free so pm can read it.
- **UI (`public/admin/projects/index.html`):** list (active vs archived + toggle, per-card Archive/Delete/Restore) → drill-in with **in-place** editors: Overview (exec grid + editable narrative + multi Project-Hub link + Create/refresh Hub card), Budget & Schedule (add/edit/archive lines inline; totals refresh live; tab preserved on save), Grants (list ALL village grants, Link/Unlink/Move-here), Volunteers (checkbox list "which groups feed this project" + hour rate + live hours), Reports (print steering-summary + detailed tracker via @media print, Schedule/Budget CSV via cocon-export).
- **Roles:** new **Village PM** (`<village>:pm`) — manages projects/budgets/schedule/links; **money read-only** (can't set grant-request amount, can't delete). Wired through `_auth.js` (generic matching), `/admin/` Access dropdowns + role×module **matrix** (`MATRIX_ROLES = admin·treasurer·pm·steward·viewer`), `village-modules.js`, and the shared `admin-nav.js` switcher (Projects pill).
...
- **Session 20 (Aug 26):** **VF1 Village Admin portal** — decoupled village administration (All Villages, packages Foundation/Interactive/Complete, module toggles, role×module matrix, Access) into a dedicated `/admin/` portal; survey-admin cut back to surveys-only. Recorded in memory ([[vf1-village-admin-portal]]) + the signal log; not separately expanded here.
- **Session 21 (Aug 28):** **Projects system of record + Village PM role + steward Identity fix + console tidy** (6 merges to main, all verified live). (1) **Steward "user not found" 404** — self-healing sign-in, "Remove" now revokes roles (not hard-delete), Resend invite, invite links → /admin/ (see section above). (2) **Projects module** `/admin/projects/` (see section above): list→drill-in with in-place Budget/Schedule/Grants/Volunteers editing, print+CSV reports, multi Project-Hub links, bidirectional group↔project linking (picker sources the FULL volunteer-card catalogue), project archive/delete. (3) **Village PM role** + per-role module matrix (`pm` column). (4) **Volunteer-hub enrichments** (`/admin/volunteers/my/`): per-group QR sign-up + printable poster + SMS/WhatsApp share (self-hosted `/admin/vendor/qrcode.min.js`), richer roster (tap-to-contact, member badge, hours, last-active, thank-you), working-bee tick-from-roster + printable sign-in sheet, milestones/leaderboard. (5) **Survey-admin sidebar tidy** — removed redundant All Villages↗/Access↗ (now in shared nav + hub), Playbook→"Survey guide". (6) **Header logo** tagline "Pacific Palms Community" → "A Pacific Palms Community" (it's CSS text, not an image). **Delivery note:** built via API-contract-first + one subagent per UI file + an adversarial review pass (caught a PM CSV 403 pre-merge); every change branch→preview→verify→merge-FF. 6 test volunteers seeded on the Landcare card for testing (delete when done).
